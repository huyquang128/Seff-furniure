...
function MyFavorite() {
    ...
    const orders = orderRedux.orders !== null && orderRedux.orders[0];
        ...
        <div className="grid grid-cols-4 gap-y-10 gap-5 max-lg:grid-cols-2  w-full max-sm:grid-cols-1">
            {orders?.products?.map((product) => (
                <div key={product._id} className="relative cursor-pointer">
                    <div
                        onMouseEnter={() => handleHoverProduct(product._id)}
                    ...
                    >
                        ...
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{
                                opacity: isHideAct === product._id ? 1 : 0,
                            }}
                            transition={{ duration: 0.3 }}
                            className={`${
                                isHideAct === product._id ? '' : 'hidden'
